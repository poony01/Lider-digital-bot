...
🧾 Nome: ${nomePix}`, { parse_mode: "Markdown" });
      ...
      if (userId === OWNER_ID) {
        ...
        if (text.startsWith("/zerarsaldo")) {
          ...
          if (!isNaN(id)) {
            await zerarSaldo(id);
            await bot.sendMessage(chat.id, `✅ Saldo do ID \`${id}\` zerado.`, { parse_mode: "Markdown" });
          } else {
            await bot.sendMessage(chat.id, "❌ ID inválido. Use: /zerarsaldo ID", { parse_mode: "Markdown" });
          }
          return res.end();
        }
        ...
        if (text.startsWith("/enviar")) {
          ...
          if (!destinoId || !mensagem) {
            await bot.sendMessage(chat.id, "❌ Uso: /enviar ID mensagem", {
              parse_mode: "Markdown"
            });
            return res.end();
          }
        ...
        if (text.startsWith("/broadcast")) {
          ...
          if (!mensagem) {
            await bot.sendMessage(chat.id, "❌ Envie assim: /broadcast sua mensagem");
            return res.end();
          }
          ...
          for (const u of todos) {
            try {
              ...
              console.log(`Erro ao enviar para ${u.user_id}:`, err.message);
            }
          }
